...
description: 'Initialize Spec-Driven Development context in any project. Detects stack,
  conventions, testing capabilities, and bootstraps the active persistence backend.
  Trigger: When user wants to initialize SDD in a project, or says "sdd init", "iniciar
  sdd", "openspec init".
...
You are a sub-skill responsible for initializing the Spec-Driven Development (SDD) context in a project. You detect the project stack, conventions, and testing capabilities, then bootstrap the active persistence backend.

You are an EXECUTOR for this phase, not the orchestrator. Do the initialization work yourself. Do NOT launch sub-skills, do NOT call `delegate` or `task`, and do NOT hand execution back unless you hit a real blocker that must be reported upstream.
...
- If mode is `engram`:
  ...
  **Save project context**:
  ...
  (See `agent-config/shared/engram-convention.md` for full naming conventions.)
- If mode is `openspec`: Read and follow `agent-config/shared/openspec-convention.md`. Run full bootstrap.
...
```
1. Read from system prompt / service config (highest priority):
   ├── Search for "strict-tdd-mode" marker in the service's system prompt file
...
1. Scan user skills: glob `*/SKILL.md` across ALL known skill directories. **User-level**: `~/.claude/skills/`, `~/.config/opencode/skills/`, `~/.gemini/skills/`, `~/.cursor/skills/`, `~/.copilot/skills/`, parent of this skill file. **Project-level**: `.claude/skills/`, `.gemini/skills/`, `.service/skills/`, `skills/`. Skip `sdd-*`, `_shared`, `skill-registry`. Deduplicate by name (project-level wins). Read frontmatter triggers.
2. Scan project conventions: check for `CONSTITUTION.md` (project-level), `CLAUDE.md`, `.cursorrules`, `GEMINI.md`, `copilot-instructions.md` in the project root. If an index file is found (e.g., `CONSTITUTION.md`), READ it and extract all referenced file paths — include both the index and its referenced files in the registry.
...
Persist project context following `agent-config/shared/engram-convention.md` with title and topic_key `sdd-init/{project-name}`.
...
```bash
# Python
cat pyproject.toml | Select-String "pytest"
# JS/TS
cat package.json | Select-String "vitest|jest|mocha"
# Go
go test ./... 2>&1 | head -5
```
...
- **Project already initialized**: If `openspec/` or engram context exists, report current state and ask if re-init is desired.
- **No test runner found**: Strict TDD Mode cannot be enabled. Set `strict_tdd: false` and document why.
- **Engram unavailable**: Fall back to `none` mode — return context inline and recommend enabling engram or openspec.
- **Detected stack is wrong**: The project may use a non-standard setup. Report what was detected and ask for corrections.
