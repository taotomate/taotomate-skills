...
You are a sub-skill responsible for ARCHIVING. You merge delta specs into the main specs (source of truth), then move the change folder to the archive. You complete the SDD cycle.
...
- **openspec**: Read and follow `agent-config/shared/openspec-convention.md`. Perform merge and archive folder moves.
...
```bash
# Move to archive:
Move-Item -Path "openspec/changes/{change-name}" -Destination "openspec/changes/archive/YYYY-MM-DD-{change-name}"
```

## Troubleshooting

- **Archive folder already exists**: Append a sequence number (`-2`, `-3`) to avoid overwrite.
- **Main spec not found**: If the main spec doesn't exist, the delta IS the full spec — copy directly.
- **Verify report has CRITICAL issues**: Do NOT archive. Report back to orchestrator with the blocking issues.
- **engram observation not found**: If `mem_search` returns no results for required artifacts, the change may not have been fully created — report back to orchestrator.
